...
enum HookInstaller {
    ...
    /// Cursor's stop hook fires when an agent turn ends, with JSON on stdin.
    /// The registered command is a wrapper script (see Paths.cursorWrapper)
    /// that execs the installed helper with the "cursor" subcommand.
    static func cursorWrapperContent() -> String {
        """
        #!/bin/bash
        # Generated by NotCode. Cursor invokes this on its stop hook; the JSON
        # payload on stdin flows through exec to the helper.
        exec \(shellEscape(Paths.installedHelper.path)) cursor
        """
    }

    static func cursorStatus() -> Status {
        guard let object = readCursorHooks() else { return .notInstalled }
        let hooks = object["hooks"] as? [String: Any] ?? [:]
        let entries = hooks["stop"] as? [[String: Any]] ?? []
        return containsOurCursorHook(entries) ? .installed : .notInstalled
    }

    static func installCursor() throws {
        try FileManager.default.createDirectory(
            at: Paths.cursorWrapper.deletingLastPathComponent(), withIntermediateDirectories: true)
        try cursorWrapperContent().write(to: Paths.cursorWrapper, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes(
            [.posixPermissions: 0o755], ofItemAtPath: Paths.cursorWrapper.path)

        let merged = mergedCursorHooks(into: readCursorHooks() ?? [:])
        try backup(Paths.cursorHooks)
        try writeJSON(merged, to: Paths.cursorHooks)
    }

    static func uninstallCursor() throws {
        defer { try? FileManager.default.removeItem(at: Paths.cursorWrapper) }
        guard var object = readCursorHooks(),
              var hooks = object["hooks"] as? [String: Any],
              var entries = hooks["stop"] as? [[String: Any]] else { return }
        entries.removeAll { ($0["command"] as? String)?.contains(helperMarker) == true }
        if entries.isEmpty { hooks.removeValue(forKey: "stop") } else { hooks["stop"] = entries }
        object["hooks"] = hooks
        try backup(Paths.cursorHooks)
        try writeJSON(object, to: Paths.cursorHooks)
    }

    /// Pure merge: adds our stop hook to a hooks.json object, preserving
    /// everything already there. Idempotent.
    static func mergedCursorHooks(into object: [String: Any]) -> [String: Any] {
        var object = object
        if object["version"] == nil { object["version"] = 1 }
        var hooks = object["hooks"] as? [String: Any] ?? [:]
        var entries = hooks["stop"] as? [[String: Any]] ?? []
        if !containsOurCursorHook(entries) {
    ...
    private static func readCursorHooks() -> [String: Any]? {
        guard let data = try? Data(contentsOf: Paths.cursorHooks) else { return nil }
        return (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
    }

    private static func containsOurCursorHook(_ entries: [[String: Any]]) -> Bool {
        entries.contains { ($0["command"] as? String)?.contains(helperMarker) == true }
    }
