...
You kick off a long agent run and walk away. Twenty minutes later it's been sitting there waiting for a permission click, and you're in the kitchen. NotCode fixes that loop:
...
- 🤫 **Silent while you're watching** — typing in your terminal/IDE? NotCode knows you can see the agent and stays quiet. One notification per conversation, never spam
- 💸 **Message-frugal** — WhatsApp only sends when you've actually been away from the keyboard; status updates only, never your code or task output
- 🪶 **Native Swift, ~5 MB** — no Electron, no background CPU, works even when the app isn't running
- 🔌 **One-click hook install** — safely merges into `~/.claude/settings.json` and `~/.codex/config.toml` with backups; plays nice with existing hooks (it chains, never overwrites)

Works with **Claude Code** (CLI + desktop app) and **Codex** (CLI + desktop app).

## Install

1. Download the DMG from **[notcode.rairai.xyz](https://notcode.rairai.xyz)** (or grab the [latest release](https://github.com/vadi25/notcode/releases/latest/download/NotCode.dmg))
2. Drag **NotCode** to Applications and open it
3. First launch on an unsigned build: macOS will balk. Go to **System Settings → Privacy & Security → "Open Anyway"** (one time only — we're an open-source app without Apple's $99/yr notarization, for now)
...
4. **Click "Install hooks"** — done. Run any agent task and go make coffee ☕
